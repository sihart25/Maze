    ...
    if (token == "Texture") {
        ...
        while (token != CLOSING_BRACE) {
            ...
            if (token == "Filename") {
                ...
                in /*>> boolalpha*/ >> texture->_mipmap /*>> noboolalpha*/ >> token;
            }
            else {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, bitmap.width(), bitmap.height(), 0, GL_RGB, GL_UNSIGNED_BYTE, bitmap.image());
        gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, bitmap.width(), bitmap.height(), GL_RGB, GL_UNSIGNED_BYTE, bitmap.image());
